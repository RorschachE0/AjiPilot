...
# -*- coding: utf-8 -*-
"""
ajiasu — 最简 Web 面板（UI 美化版 - 逻辑未变）

核心逻辑完全保留原版，仅替换 index() 中的 HTML 前端代码。
"""
...
try:
    _autoswitch_env = int(os.environ.get("AJIASU_AUTOSWITCH_SEC", str(1 * 300)))
except Exception:
    _autoswitch_env = 300  # 6*3600
...
/* Components */
h2 { font-size: 1.5rem; font-weight: 700; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }
h2 span { font-size: 0.8em; opacity: 0.6; font-weight: 400; }
...
.btn-primary { background: var(--accent-color); border-color: var(--accent-color); color: white; font-weight: 600; }
.btn-primary:hover:not(:disabled) { background: var(--accent-hover); color: white; }
.btn-danger { color: var(--danger-color); border-color: var(--border-color); }
.btn-danger:hover { background: rgba(239, 68, 68, 0.1); border-color: var(--danger-color); }

/* Status Bar */
.status-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; }
.status-item { background: rgba(0,0,0,0.2); padding: 10px 15px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; }
.status-label { color: var(--text-secondary); font-size: 12px; }
.status-value { font-family: monospace; font-weight: 600; font-size: 1.1em; }
.badge { padding: 2px 8px; border-radius: 99px; font-size: 12px; background: var(--border-color); }
...
.table-wrapper {
  ...
  border-radius: 8px;
  border: 1px solid var(--border-color);
  max-height: 400px; /* Scrollable body */
}
table { width: 100%; border-collapse: collapse; text-align: left; }
th { background: rgba(0,0,0,0.3); color: var(--text-secondary); font-weight: 600; position: sticky; top: 0; backdrop-filter: blur(5px); z-index: 10; }
...
<div class="container">
  <div class="flex-row space-between" style="margin-bottom: 20px;">
    <h2>Ajiasu <span>Pro Panel</span></h2>
    <div class="flex-row">
       <span class="status-label">AutoSwitch: ON</span>
    </div>
  </div>
  ...
  <div class="status-grid" style="margin-bottom: 20px;">
    <div class="status-item">
      <div>
        <div class="status-label">CURRENT NODE</div>
        <div class="status-value" id="selLabel" style="color: var(--accent-color)">--</div>
      </div>
      <div id="status-indicator" style="width:10px; height:10px; background: var(--border-color); border-radius:50%"></div>
    </div>
    <div class="status-item">
      <div>
        <div class="status-label">EXTERNAL IP</div>
        <div class="status-value" id="extIp">Checking...</div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    <div class="flex-row space-between">
      <div class="flex-row">
        ...
        <select id="protocol">
          <option value="lwip" selected>LWIP (Default)</option>
          <option value="tcp">TCP</option>
          <option value="udp">UDP</option>
          <option value="proxy">Proxy</option>
        </select>
        <button id="btn-connect" class="btn-primary" disabled>Connect Selected</button>
      </div>
    ...
    <div style="margin-top: 10px; color: var(--text-secondary); font-size: 12px;">
      * Operations automatically clean up zombie processes.
    </div>
  </div>
  ...
  <div class="card" style="padding: 0; overflow: hidden;">
    <div class="table-wrapper">
      <table>
        ...
        <tbody id="tbody">
          <tr><td colspan="5" style="text-align:center; padding: 40px; color: #666;">Click "Refresh List" to load nodes</td></tr>
        </tbody>
      </table>
    </div>
    <div style="padding: 10px 16px; background: rgba(0,0,0,0.2); border-top: 1px solid var(--border-color); font-size: 12px; color: var(--text-secondary);">
      Total Nodes: <span id="count" style="color: white; font-weight: bold;">0</span>
    </div>
  </div>

  <div class="terminal" id="out">> System Ready.</div>
</div>
...
function setSelectedLabel(lbl) {
  $('#selLabel').textContent = lbl || 'None';
  $('#status-indicator').style.background = lbl ? 'var(--success-color)' : 'var(--border-color)';
}

function setExternalIp(ip) {
  $('#extIp').textContent = ip || 'Unknown';
}

function render(nodes) {
  tbody.innerHTML = '';
  if(nodes.length === 0) {
     tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding: 20px;">No nodes found</td></tr>';
     return;
  }
  nodes.forEach(n => {
    const tr = document.createElement('tr');
    // Check if this node matches current or selected
    const isCurrent = (CURRENT && CURRENT.label === n.label);
    const isChecked = isCurrent || (SELECTED === n.label);

    if(isCurrent) tr.style.background = 'rgba(59, 130, 246, 0.15)'; // Highlight current

    tr.innerHTML = `
      <td><input type="radio" name="pick" value="${n.label}" ${isChecked ? 'checked' : ''}></td>
      <td class="mono">${n.id}</td>
      <td class="${n.status === 'ok' ? 'status-ok' : ''}">${n.status}</td>
      <td>${n.city}</td>
      <td><span class="badge ${isCurrent ? 'active' : ''}">${n.label}</span></td>`;

    tr.onclick = (e) => {
      // Allow clicking row to select radio
      if(e.target.type !== 'radio') {
         tr.querySelector('input').checked = true;
      }
      SELECTED = n.label;
      // Update UI selection immediately purely for visual feedback
      document.querySelectorAll('tr').forEach(r => r.style.background = '');
      if(isCurrent) tr.style.background = 'rgba(59, 130, 246, 0.15)';

      $('#btn-connect').disabled = false;
      $('#btn-connect').textContent = `Connect to ${n.city}`;
    };
...
async function refreshStatus() {
  try {
    ...
    if (CURRENT && CURRENT.label) {
      ...
      // Auto-enable disconnect if connected
      $('#btn-disconnect').disabled = false;
    } else {
      setSelectedLabel(null);
    }

    // Re-render list if we have nodes to update active badges
    if (NODES.length) {
      render(NODES);
    }
...
async function fetchIp() {
  try {
    $('#extIp').classList.add('animate-pulse');
    const r = await fetch('/api/external_ip');
    const d = await r.json();
    if (d.ok) {
      setExternalIp(d.ip);
    }
    ...
    $('#extIp').classList.remove('animate-pulse');
  }
}
...
$('#btn-list').onclick = async () => {
  ...
  try {
    ...
    if (!d.ok) {
      log('Error: ' + (d.error || 'ajiasu unavailable'));
      return;
    }
    NODES = d.nodes || [];
    render(NODES);
...
$('#btn-connect').onclick = async () => {
  ...
  log(`Starting connection: ${SELECTED} [${protocol}]...`);
  ...
  try {
    ...
    const r = await fetch('/api/connect', {
      ...
      body: JSON.stringify({ label: SELECTED, protocol })
    });
    const d = await r.json();
    log('Result: ' + JSON.stringify(d));

    if (d.ok) {
      CURRENT = { label: SELECTED, protocol: protocol, pid: d.pid };
...
def run_selftests() -> Dict[str, Any]:
    ...
    def _c2():
        n0 = parsed["nodes"][0]
        assert n0["id"] == "vvn-5871-9238" and n0["status"] == "ok" and n0["city"] == "苏州" and n0["num"] == 33 and n0[
            "label"] == "苏州 #33"
    ...
    def _c4():
        s = parsed["summary"]
        assert s.get("website") == "https://www.91ajs.com" and s.get("login_result") == "OK" and s.get(
            "membership") == "爱加速会员" and "2025" in s.get("expiration", "")
